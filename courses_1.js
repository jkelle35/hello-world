...
function findByPrefix(prefix) {
    return courses.filter(cource => cource.prefix === prefix);
}


//To do: implement updateById(id, course)
function updateById(id, course) {
    if(courses.filter(cource => cource.id === id) != 0) {
        let x = courses.indexOf(courses.find(course => course.id ===id))
        courses.splice(x, 1, course);
        return true;
    } else {
        return false;
    }
